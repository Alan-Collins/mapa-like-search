...
#######STEP 1 PROCESS NCBI SUMMARY FILES####### 
  #Download LapD_MoxY_N domain containing strain summary list: https://www.ncbi.nlm.nih.gov/protein?linkname=cdd_protein_specific_2&from_uid=318615
    #8700 hits as of 10-20-2018
  
  #Download LapG comtaining strain summary list: https://www.ncbi.nlm.nih.gov/protein?linkname=cdd_protein&from_uid=310549
    #20685 hits as of 10-20-2018

  ###if you download another format the pNCBI summary function won't work###

####CONVERT SUMMARY FILES#####
pSUM.lapD<-pNCBISummary("/Users/jarrodsmith/Desktop/For Alan/LapD_.txt") 
pSUM.lapG<-pNCBISummary("/Users/jarrodsmith/Desktop/For Alan/LapG_.txt")
##############################

#######STEP 2 - Identify LapGD-encoding bugs, do a little analysis########
lapD.organism.tally<- length(unique(pSUM.lapD$ORG)) #how many unique genomes have LapD-like protein? 2688
lapG.organism.tally<- length(unique(pSUM.lapG$ORG)) #how many unique genomes have LapG-like protein? 5415
lapGD.organisms<-intersect(pSUM.lapG$ORG,pSUM.lapD$ORG) #select organisms found on both lists. 2569. automatically removes replcates so no need for unique function.
...
genome.url<-"ftp://ftp.ncbi.nlm.nih.gov/genomes/GENOME_REPORTS/prokaryotes.txt" ##download this file and save it as "ALL_genomes_proks.csv". you can do this in R if you please. read it below
all.genomes<-read.csv("Source Files/ALL_genomes_proks.csv", stringsAsFactors = FALSE, header=TRUE) #file contains all bacterial genomes as of 9.17.16
...
lapd.g.species.info<-data.frame(all.genomes[get.rows,c(1,7,18,19)]) #create dataset of only organisms and info of interest
...
write.csv(new.d,"Output/dg.genome.url.files")
...
setwd("/Users/jarrodsmith/Desktop/WORKS IN PROGRESS/BIOINFORMARICS/Final_CDD_Run/protein_faa/") #change this
for (i in 1:length(new.d$organism)){ #
  message("Downloading: ", new.d$prt[i],"\n")
  download.file(new.d$prt[i], destfile = paste(new.d$organism[i], "_protein.faa.gz", sep=""))   #download genome faa
  Sys.sleep(10)
...
substrates2<-subset(new_my.table[which(new_my.table$lapG==TRUE),], stringsAsFactors=FALSE)
